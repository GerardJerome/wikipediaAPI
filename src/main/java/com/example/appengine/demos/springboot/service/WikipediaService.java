...
public class WikipediaService {
	...
	private final String searchUrl = "https://en.wikipedia.org/w/api.php?action=opensearch&format=json&search=";
	private final String contentUrl = "https://en.wikipedia.org/w/api.php?action=query&prop=revisions&rvprop=content&format=json&titles=";
	...
	public String getContent(String titre) throws IOException {
		...
		return json.getAsJsonObject("query").getAsJsonObject("pages").getAsJsonObject().get(json.getAsJsonObject("query").getAsJsonObject("pages").entrySet().iterator().next().getKey()).getAsJsonObject().get("revisions").getAsJsonArray().get(0).getAsJsonObject().get("*").toString();
	}
